...
import {Home,UserRound,BriefcaseBusiness,Award,GraduationCap,Mail, Github, Linkedin, Moon, Sun, ArrowRight, Download, ExternalLink, Menu, X, CheckCircle2, MapPin, Code2} from 'lucide-react';
import './styles.css';
...
function StatsRow(){
  return (
    <div className="stats">
      <div><b>1</b><span>PROJECT SHIPPED</span></div>
      <div><b>600+</b><span>OJT HOURS</span></div>
      <div><b>2</b><span>CERTIFICATIONS</span></div>
    </div>
  );
}
    ...
    <div className={dark?'app dark':'app'}>
      ...
      <main>
        ...
        <section id="home" className="hero page">
          <div className="hero-top">
            <div className="hero-copy">
              <div className="eyebrow">01 — JUNIOR SOFTWARE DEVELOPER</div>
        ...
        <section id="about" className="page">
          <Header n="02" title="About Me" text="A quick profile record covering my objective, strengths, and practical experience."/>
          ...
          <div className="experience glass">
            ...
            <ul>
              <li>Performed data entry and encoding of member and billing records.</li>
              <li>Processed and organized paper-based documents for daily workflow.</li>
              <li>Automated repetitive data tasks with Microsoft Excel VBA.</li>
              <li>Assisted staff with general clerical and office support tasks.</li>
            </ul>
          </div>
        </section>
        ...
        <section id="projects" className="page">
          <Header n="03" title="My Projects" text="Selected work that demonstrates my practical skills and what I can build."/>
          ...
          <div className="project-grid">
            {projects.slice(1).map((p,i)=>
              <motion.article whileHover={{y:-6}} className="glass project-card" key={p.title}>
                <img src={A+p.image}/>
                <span className="mini-label">PROJECT / {String(i+2).padStart(2,'0')}</span>
                <h3>{p.title}</h3>
        ...
        <section id="certifications" className="page">
          <Header n="04" title="Certifications" text="Verified records supporting my technical training and learning."/>
          <div className="cert-grid">
            {certs.map((c,i)=>
              <motion.article whileHover={{y:-6}} className="glass cert-card" key={c.title}>
                <img src={A+c.image}/>
                <div>
                  <span className="mini-label">CERT / {String(i+1).padStart(2,'0')}</span>
                  <h3>{c.title}</h3>
                  <p>{c.sub}</p>
        ...
        <section id="education" className="page">
          <Header n="05" title="Education" text="My academic background and training that built my foundation."/>
          <div className="education glass">
            <div className="edu-visual"><GraduationCap size={72}/><span>LEARN<br/>BUILD<br/>GROW</span></div>
            <div className="timeline">
              {[
                ['Bachelor of Science in Computer Science','President Ramon Magsaysay State University, Iba, Zambales','2025 – 2026'],
                ['Senior High School','Lyceum of Western Luzon Inc., Botolan, Zambales','2020 – 2022'],
                ['Junior High School','Porac Integrated School','2016 – 2020'],
                ['Elementary','Porac Elementary School','2010 – 2016']
              ].map(([a,b,c])=>
                <div className="timeline-item" key={a}>
                  <i/>
                  <div><h3>{a}</h3><p>{b}</p><span>{c}</span></div>
                </div>
              )}
            </div>
          </div>
        ...
        <section id="contact" className="page contact">
          <Header n="06" title="Let's Work Together" text="Open to IT support, data entry, administrative, and software development opportunities."/>
          <div className="contact-grid">
            <div className="glass contact-card">
              <a href="mailto:lansii0430@gmail.com"><Mail/><div><span>Email</span><b>lansii0430@gmail.com</b></div></a>
              <a href="tel:+639637079906"><Mail/><div><span>Phone</span><b>0963 707 9906</b></div></a>
              <div><MapPin/><div><span>Location</span><b>Porac, Botolan, Zambales</b></div></div>
...
function Header({n,title,text}){
  return (
    <div className="section-head">
      <div className="eyebrow">{n} — {title.toUpperCase()}</div>
      <h2>{title}</h2>
      <p>{text}</p>
    </div>
  );
